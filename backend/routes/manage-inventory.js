...
router.get("/:inventory_id", httpCoWrap(function*(req, res, next){
...
router.put("/:inventory_id",isAdmin, httpCoWrap(function*(req, res, next){
	let inventory = yield InventoryManagementService.updateInventoryItem(req.params.inventory_id,req.body);
	res.send(inventory);
}));

router.delete("/:inventory_id",isAdmin, httpCoWrap(function*(req, res, next){
