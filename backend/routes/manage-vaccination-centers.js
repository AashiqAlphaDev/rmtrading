...
router.get("/:vaccination_center_id/admins", co.wrap(function*(req, res, next){
	let diseases = yield VaccinationCenterManagementService.admins(req.params.vaccination_center_id);
	res.send(diseases);
}));

router.post("/:vaccination_center_id/admins", isAdmin,co.wrap(function*(req, res, next){
	let disease = yield VaccinationCenterManagementService.createAdmin(req.params.vaccination_center_id,req.body);
	res.send(disease);
}));
