...
let Index = withStyles((theme) => {
    ...
    componentWillMount() {
        ...
        this.props.dispatch({type: REQUEST_PET_TYPE_FETCH, payload:{pet_type_id:this.props.match.params.pet_type_id}});
    }
			...
			<Paper className={classes.paperPage}>
				<Layout alignItems={"flex-end"}>
					...
					<form onSubmit={(e)=>{
					    ...
					    this.props.dispatch({type:REQUEST_UPDATE_PET_TYPE, payload:{pet_type_id:this.props.match.params.pet_type_id,data:{$push:{vaccination_fields:{name, field_type}}}}})
                    }}>
                        ...
                        <InputContainer label={"Field Type"}>
                            <Select value={this.state.field_type} onChange={(event)=>{this.setState({field_type:event.target.value})}}>
