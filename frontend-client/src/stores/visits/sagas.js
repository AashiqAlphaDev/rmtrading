...
import {QUERY_VISITS,	REQUEST_ADD_BIOMETRIC,
	...
	ADD_BIOMETRIC_FAILED,QUERY_VISITS_SUCCEEDED,QUERY_VISITS_FAILED} from "./actions";

import {ADD_VISIT_FAILED, ADD_VISIT_SUCCEEDED, REQUEST_ADD_VISIT} from "../pet-types/actions";
...
let fetchVisits = function* (action) {
	try {
		...
		if (response.ok) {
			yield put({type: QUERY_VISITS_SUCCEEDED, payload: yield response.json()});
		}
		else {
